...
function App() {
  ...
  const [message, setMessage] = useState('Tracez un ligne Parallél avec la ligne tracé ');
  const [initialLine, setInitialLine] = useState(null);

  // Génère une ligne initiale avec une pente aléatoire à chaque démarrage ou réinitialisation
  useEffect(() => {
    const startX = Math.random() * 200;  // Vous pouvez ajuster ces valeurs
    const startY = Math.random() * 200;
    const endX = startX + (Math.random() - 0.5) * 300;  // Pente aléatoire
    const endY = startY + (Math.random() - 0.5) * 300;

    setInitialLine({ start: [startX, startY], end: [endX, endY] });
  }, []);
...
const handleCheck = () => {
    ...
    if (checkMode === 'parallel') {
        if (areParallel(initialLine, lines[0])) {
        ...
        if (arePerpendicular(initialLine, lines[0]))  {
...
>
  <svg style={{ width: '100%', height: '100%' }}>
    ...
    {lines.map((line, index) => (
     ...
     {initialLine && (
            <line
              x1={initialLine.start[0]} 
              y1={initialLine.start[1]}
              x2={initialLine.end[0]} 
              y2={initialLine.end[1]}
              stroke='#000'
              strokeWidth="3"
            />
          )}
